...
from throughster.factory import ModelInterface, create_interface
...
def _create_interface_untimed_cache(**kwargs) -> ModelInterface:
    ...
    throughster builds `FileCache(serializer=PickleSerializer(), basedir=...)`
    without a timeout, so it inherits aiocache's BaseCache default of 5s, which
    wraps every cache get/set in `asyncio.wait_for`. On a Lustre
    $THROUGHSTER_CACHE_DIR a tail-latency stall past 5s raises a bare
    asyncio.TimeoutError -- which tenacity does not retry (its predicate covers
    only RateLimitError and httpx errors) and throughster's
    `except* StructuredResponseError` does not catch, so it escapes the anyio
    task group and kills the whole stage. That is the failure that ended job
    176217145 at row 518/550 of `locate`. The response has already been
    generated by then: a slow *cache write* must never discard it.

    0 disables the wrapper (aiocache treats 0/None as "no timeout"). The timeout
    protects nothing anyway -- aiofiles runs the I/O in a thread-pool executor,
    so cancelling the await cannot stop the write. PBS walltime is the real
    bound. Mutating the attribute is what works here: `_handle_cached_function`
    captures the cache object by reference in ModelInterface.__init__, so
    replacing `client.cache` afterwards would be a no-op, and `API.timeout`
    re-reads `self.timeout` on every call.
    """
...
def _init_client_fn(
    ...
    **kwargs,
    ...
    # NOTE: a `partial` of a module-level function, NOT a closure -- the agent
    # holding this factory is dill-pickled out to every datasets.map(num_proc)
    # worker, so keep it trivially serializable.
    return partial(
        _create_interface_untimed_cache,
        provider=provider,
        api_base=api_base,
